...
class Mask_free_DC(nn.Module):
    def __init__(self, dim=28, num_iterations=1):
        ...
        for _ in range(num_iterations):
            self.denoisers.append(
                HPT(in_dim=dim+1, out_dim=dim, dim=dim, num_blocks=[1,1,1]),
            )
    ...
    def forward(self, y, input_mask=None, rgb=None, rgb_s=None):
        ...
        z = self.z_init(torch.cat([z], dim=1))
        ...
        for i in range(self.num_iterations):
            ...
            if i<self.num_iterations-1:
                z = shift_3d(z)
        return z[:, :, :, 0:256], z[:, :, :, 0:256]


# if __name__=="__main__":
#     from thop import profile
#     dim=28
#     model = Mask_free_DC(dim=dim, num_iterations=9).cuda()
